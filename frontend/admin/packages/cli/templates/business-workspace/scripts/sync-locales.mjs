...
for (const entry of existsSync(modulesRoot) ? readdirSync(modulesRoot, { withFileTypes: true }) : []) {
  ...
  if (!locales.includes('zh-CN')) throw new Error(workspaceMessage('locale_missing_default', { module: entry.name }))
  if (expectedLocales && locales.join(',') !== expectedLocales) throw new Error(workspaceMessage('locale_set_mismatch', { module: entry.name }))
  expectedLocales = locales.join(',')
  const messages = Object.fromEntries(locales.map(locale => {
    if (!/^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(locale)) throw new Error(workspaceMessage('locale_code_invalid', { locale }))
  ...
  for (const locale of locales) {
    if (Object.keys(messages[locale]).sort().join('\0') !== keys) throw new Error(workspaceMessage('locale_keys_mismatch', { module: entry.name, locale }))
  }
  ...
  else if (!existsSync(file) || readFileSync(file, 'utf8') !== content) throw new Error(workspaceMessage('locale_bundle_outdated', { file }))
}
