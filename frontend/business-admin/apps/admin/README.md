...
`business-admin` 的管理端宿主。该包私有且不实现业务，默认先组合 `@liujitcn/kratos-admin-system`，再组合自有 module **MODULE_PACKAGES** 和所选的其他业务 module。
...
```bash
pnpm --filter @business/admin-app dev
pnpm --filter @business/admin-app type:check
pnpm --filter @business/admin-app build
```
