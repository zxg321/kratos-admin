...
すべての `Taro.request`、ファイルアップロード、SSE リクエストは `Accept-Language` を送信します。動的メニューはバックエンドが解析したタイトルを使用し、訳がない場合は主言語へフォールバックします。新しい言語を追加する場合はバックエンドと 3 workspace の言語パックを同期し、ルートで `make i18n` を実行します。
