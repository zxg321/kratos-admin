...
H5 生产构建默认输出到 `backend/web/taro-app` 并使用 `/taro-app/` 公共路径。开发产物分别写入 `dist/dev/h5` 和 `dist/dev/mp-weixin`；微信小程序生产产物写入 `dist/build/mp-weixin`。微信开发者工具导入宿主时默认使用开发目录，发布时导入 `dist/build/mp-weixin`。H5 和微信小程序可以同时开发，页面装配由 runner 共享。自定义 `KRATOS_TARO_OUTPUT_ROOT` 优先于默认目录。
