...
for (const entry of existsSync(modulesRoot) ? readdirSync(modulesRoot, { withFileTypes: true }) : []) {
  ...
  if (!locales.includes('zh-CN')) throw new Error(workspaceMessage('missing_default_locale', { module: entry.name }))
  ...
  for (const locale of locales) {
    if (Object.keys(messages[locale]).sort().join('\0') !== keys) throw new Error(workspaceMessage('key_set_mismatch', { module: entry.name, locale }))
  }
  ...
  else if (!existsSync(file) || readFileSync(file, 'utf8') !== content) throw new Error(workspaceMessage('bundle_outdated', { file }))
}
