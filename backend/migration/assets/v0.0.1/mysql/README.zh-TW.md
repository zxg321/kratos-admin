...
- 系統參數與參數管理同屬基礎管理：前者透過專用表單編輯配置，後者維護同一套配置的 Key、類型和值。資料備份和資料封存各自直接包含配置、記錄及還原記錄。系統維運包含執行監控、執行日誌、快取查詢和資料庫遷移記錄（原升級歷史）；開發工具只顯示介面文件與程式碼產生。字典資料仍為基礎管理下的隱藏頁面 `91010700`；程式碼產生的隱藏配置與預覽頁位於開發工具下，使用 `91110300` 至 `91110600`，按鈕使用 `91110201` 至 `91110209`。
