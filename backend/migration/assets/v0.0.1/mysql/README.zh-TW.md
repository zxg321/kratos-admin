...
- 安全能力：登入策略選單及其服務、按鈕權限只授予平台超級管理員；OAuth 登入方式位於登入管理下並排在線上工作階段之前，只授予系統管理員。初始化全部九種內建 Provider，原配置中的 Gitee、GitHub、Google、微信小程式保持啟用，其他 Provider 預設停用。`client_secret` 使用 `ENCRYPT` 入庫規則，`algorithm` 可選 `AES_GCM` 或 `SM4_GCM`，主表只保存不含 `ENC[]` 的對應算法密文載荷，查詢回呼恢復原文且不寫入敏感值旁表。線上工作階段及下線權限授予平台管理員與租戶管理員，租戶管理員僅限本租戶。
