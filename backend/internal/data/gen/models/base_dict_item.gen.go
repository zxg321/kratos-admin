...
// BaseDictItem mapped from table <base_dict_item>
type BaseDictItem struct {
	ID        int64                 `gorm:"column:id;type:bigint;primaryKey;autoIncrement:true;comment:字典属性ID" json:"id"`                        // 字典属性ID
	DictID    int64                 `gorm:"column:dict_id;type:bigint;not null;comment:字典ID" json:"dict_id"`                                     // 字典ID
	Value     string                `gorm:"column:value;type:character varying(50);not null;comment:字典属性值" json:"value"`                         // 字典属性值
	Label     string                `gorm:"column:label;type:character varying(100);not null;comment:字典属性标签" json:"label"`                       // 字典属性标签
	TagType   string                `gorm:"column:tag_type;type:character varying(50);comment:标签类型，用于前端样式展示（如success、warning等）" json:"tag_type"` // 标签类型，用于前端样式展示（如success、warning等）
