...
type BaseRedactStorageValue struct {
	...
	Ciphertext      []byte `gorm:"column:ciphertext;type:bytea;not null;comment:加密原文" json:"ciphertext"`                                                                                                                                 // 加密原文
	Digest          []byte `gorm:"column:digest;type:bytea;not null;index:idx_base_redact_storage_value_digest,priority:3;comment:HMAC查询摘要" json:"digest"`                                                                         // HMAC查询摘要
}
