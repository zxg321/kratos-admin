...
// BaseMenu mapped from table <base_menu>
type BaseMenu struct {
	...
	Type      int32                 `gorm:"column:type;type:integer;not null;comment:菜单类型：枚举【BaseMenuType】" json:"type"`                                                                                         // 菜单类型：枚举【BaseMenuType】
	Path      string                `gorm:"column:path;type:character varying(1024);not null;comment:路径,当其类型为'按钮'的时候对应的按钮权限base:user:create" json:"path"`                                                        // 路径,当其类型为'按钮'的时候对应的按钮权限base:user:create
	Name      string                `gorm:"column:name;type:character varying(255);comment:路由命名，然后我们可以使用 name 而不是 path 来传递 to 属性给 <router-link>。如果需要开启缓存，需保证页面 defineOptions 中的 name 与此处一致，建议使用驼峰。" json:"name"` // 路由命名，然后我们可以使用 name 而不是 path 来传递 to 属性给 <router-link>。如果需要开启缓存，需保证页面 defineOptions 中的 name 与此处一致，建议使用驼峰。
	Component string                `gorm:"column:component;type:character varying(255);comment:前端页面组件" json:"component"`                                                                                        // 前端页面组件
	Redirect  string                `gorm:"column:redirect;type:character varying(1024);comment:重定向地址" json:"redirect"`                                                                                          // 重定向地址
