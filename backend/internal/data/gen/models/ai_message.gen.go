...
type AiMessage struct {
	...
	Status        int32                 `gorm:"column:status;type:smallint;not null;comment:消息生成状态：枚举【AiMessageStatus】" json:"status"`                                                                                                               // 消息生成状态：枚举【AiMessageStatus】
	CreatedAt     time.Time             `gorm:"column:created_at;type:timestamp;not null;index:idx_ai_message_tenant_id_session_id_created_at,priority:3;comment:创建时间" json:"created_at"`                                                            // 创建时间
