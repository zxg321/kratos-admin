...
// BaseTableArchiveRestore mapped from table <base_table_archive_restore>
type BaseTableArchiveRestore struct {
	...
	TableName_      string    `gorm:"column:table_name;type:character varying(128);not null;comment:数据表名称" json:"table_name"`                                                                  // 数据表名称
	RestoreMode     int32     `gorm:"column:restore_mode;type:integer;not null;comment:恢复模式：枚举【BaseTableArchiveRestoreMode】" json:"restore_mode"`                                              // 恢复模式：枚举【BaseTableArchiveRestoreMode】
	RestoreRange    string    `gorm:"column:restore_range;type:character varying(512);not null;comment:恢复范围" json:"restore_range"`                                                             // 恢复范围
	...
	Status          int32     `gorm:"column:status;type:integer;not null;index:idx_base_table_archive_restore_status,priority:1;comment:恢复状态：枚举【BaseTableArchiveRestoreStatus】" json:"status"` // 恢复状态：枚举【BaseTableArchiveRestoreStatus】
