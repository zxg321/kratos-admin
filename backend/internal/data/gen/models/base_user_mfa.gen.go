...
// BaseUserMFA mapped from table <base_user_mfa>
type BaseUserMFA struct {
	...
	UserID      int64                 `gorm:"column:user_id;type:bigint;not null;uniqueIndex:unique_base_user_mfa_user_method,priority:3;index:idx_base_user_mfa_user_id,priority:1;comment:用户ID" json:"user_id"` // 用户ID
	Method      string                `gorm:"column:method;type:character varying(20);not null;uniqueIndex:unique_base_user_mfa_user_method,priority:2;comment:认证方式：totp、webauthn" json:"method"`                 // 认证方式：totp、webauthn
	ConfirmedAt time.Time             `gorm:"column:confirmed_at;type:timestamp with time zone;comment:确认启用时间" json:"confirmed_at"`                                                                               // 确认启用时间
	...
	DeletedAt   soft_delete.DeletedAt `gorm:"column:deleted_at;type:bigint;not null;uniqueIndex:unique_base_user_mfa_user_method,priority:1;comment:删除时间;softDelete:milli" json:"deleted_at"`                     // 删除时间
}
