...
// BaseRedactRule mapped from table <base_redact_rule>
type BaseRedactRule struct {
	...
	Code          string                `gorm:"column:code;type:character varying(64);not null;uniqueIndex:unique_base_redact_rule,priority:1;comment:规则编码" json:"code"`               // 规则编码
	Name          string                `gorm:"column:name;type:character varying(100);not null;comment:规则名称" json:"name"`                                                             // 规则名称
	RuleType      string                `gorm:"column:rule_type;type:character varying(32);not null;comment:规则类型" json:"rule_type"`                                                    // 规则类型
	DefaultParams string                `gorm:"column:default_params;type:json;not null;comment:默认规则参数" json:"default_params"`                                                         // 默认规则参数
	Status        int32                 `gorm:"column:status;type:integer;not null;index:idx_base_redact_rule_status,priority:1;comment:状态：枚举【Status】" json:"status"`                  // 状态：枚举【Status】
	...
	DeletedAt     soft_delete.DeletedAt `gorm:"column:deleted_at;type:bigint;not null;uniqueIndex:unique_base_redact_rule,priority:2;comment:删除时间;softDelete:milli" json:"deleted_at"` // 删除时间
}
